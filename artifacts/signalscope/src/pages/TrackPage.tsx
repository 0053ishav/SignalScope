...
export default function TrackPage({ id }: TrackPageProps) {
      ...
      try {
        ...
        if (trackData?.commontrack_id) {
          ...
          const [lyricsData, richsyncData, analysisData] = await Promise.all([
            fetch(`/api/lyrics/${commontrackId}`)
              .then((r) => (r.ok ? r.json() : null))
              .catch(() => null),
            fetch(`/api/richSync/${commontrackId}`)
              .then((r) => (r.ok ? r.json() : null))
              .catch(() => null),
            fetch(`/api/analysis/${commontrackId}`)
              .then((r) => (r.ok ? r.json() : null))
              .catch(() => null),
          ]);
    ...
    return (
      <main className={classes.page}>
        <div className={classes.card}>
          <h1 className="text-2xl font-bold">Loading track…</h1>
        </div>
      </main>
    );
  }
  ...
  const duration =
    track.track_length > 0
      ? `${Math.floor(track.track_length / 60)}:${(track.track_length % 60)
          .toString()
          .padStart(2, "0")}`
      : null;
